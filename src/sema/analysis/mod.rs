...
impl AnalysisInfo {
    ...
    pub fn path_to_string(
        &self,
        path: &[crate::ast::PathPart],
        type_info: &crate::sema::types::TypeInfo,
    ) -> String {
    ...
    pub fn types_equal(&self, a: &crate::ast::Type, b: &crate::ast::Type) -> bool {
        match (a, b) {
            ...
            (
                crate::ast::Type::Custom(a_name, a_gens),
                crate::ast::Type::Custom(b_name, b_gens),
            ) => {
                if a_name != b_name {
                    return false;
                }
    ...
    pub fn deref_type(&self, ty: &crate::ast::Type) -> crate::ast::Type {
        match ty {
            ...
            | Type::RawPtr(inner, _)
            | Type::Managed(inner)
            | Type::Owned(inner) => self.deref_type(inner),
            Type::ThreadSafe(inner) => self.deref_type(inner),
            _ => ty.clone(),
        }
    }
    ...
    ) -> Result<crate::ast::Type, crate::error::CompilerError> {
        Err(crate::error::CompilerError::from_rich(
            chumsky::prelude::Rich::custom(_span, _msg),
        ))
    }

    pub fn analyze_function(
        &mut self,
        _func: &mut crate::ast::Function,
        _obj: Option<&str>,
        _generics: Option<&Vec<(String, Vec<String>)>>,
        _type_info: &mut crate::sema::types::TypeInfo,
