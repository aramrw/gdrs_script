    ...
    for line in source.lines() {
        ...
        if nest_level == 0 {
            ...
            if indent > last_indent {
                ...
                tokens.push((
                    Token::Indent,
                    SimpleSpan::from(line_start_offset..line_start_offset + whitespace_len),
                ));
        ...
        while let Some((i, c)) = chars.next() {
            ...
            match c {
                ...
                '(' => {
                    tokens.push((Token::ParenOpen, span(1)));
                    nest_level += 1;
                }
                ')' => {
                    tokens.push((Token::ParenClose, span(1)));
                    nest_level -= 1;
                }
                '{' => {
                    tokens.push((Token::BraceOpen, span(1)));
                    nest_level += 1;
                }
                '}' => {
                    tokens.push((Token::BraceClose, span(1)));
                    nest_level -= 1;
                }
                '[' => {
                    tokens.push((Token::BracketOpen, span(1)));
                    nest_level += 1;
                }
                ']' => {
                    tokens.push((Token::BracketClose, span(1)));
                    nest_level -= 1;
                }
                ...
                ':' => {
                    if chars.peek().map(|&(_, c)| c) == Some(':') {
                        ...
                        tokens.push((Token::Colon, span(1)));
                    }
                }
                '=' => {
                    if chars.peek().map(|&(_, c)| c) == Some('>') {
                ...
                _ if c.is_ascii_digit() => {
                    ...
                    while let Some(&(_, nc)) = chars.peek() {
                        if nc.is_ascii_alphanumeric() {
                            suffix.push(chars.next().unwrap().1);
                        } else {
                            break;
                        }
                    }
                    ...
                    match suffix.as_str() {
                        ...
                        "" => {
                            if is_float {
                                tokens.push((Token::Float(s.parse().unwrap()), span));
                            } else {
        ...
        if nest_level == 0 && !tokens.is_empty() {
            ...
            match last {
                ...
                _ => {
                    tokens.push((
                        Token::Semicolon,
                        SimpleSpan::from(last_span.end..last_span.end),
                    ));
                }
            }
        }
